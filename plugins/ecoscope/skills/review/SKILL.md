...
| Platforms and locks | inner lock solved for `win-64` (`grep -c win-64 <WF>/pixi.lock`); outer `pixi.toml` without `win-64`; both locks re-solved with CI's pinned pixi (`scripts/preflight.sh` WARNs when the local one differs; ci.md § Pixi locks) |
