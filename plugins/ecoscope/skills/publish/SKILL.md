...
- Outer `pixi.toml`: set the approved compiler pin (`wt-compiler = "==<version>"`); `platforms`
  ...
  and it is committed with the tree (`ci.md` § Pixi locks).
...
- **The two OSes you are not on** are proven by the locks, not by you: the outer `pixi.toml`
  lists all three platforms, the inner lock is solved for them (`grep -c 'win-64'
  <WF>/pixi.lock` > 0), and both were re-solved with CI's pixi (§ 3's relock step). Read the
  Windows leg's log in § 6 rather than assuming it.
